...
".join(lines), encoding='utf-8')
    ...
    def cmd_plan(args):
        ...
        # Minimal example: ensure FX + UoM + calendar + logsys
    ...
    def cmd_scaffold(args):
        ...
        # Master / LLD / Test docs placeholders
...
Unit + integration tests.", encoding='utf-8')
        # Handler template
        (out/ 'handler.py').write_text(f"""from lakehouse_fm_agent.runtime.context import Context


def handler(ctx: Context) -> None:
    """
    Handler for {fm}.
    TODO: implement logic per Master/LLD.
    """
    # ctx.current_df = ...
    pass
""", encoding='utf-8')
        ...
        for layer in topo_layers(edges):
            for fm in layer:
                ...
                if fn:
                    ...
                    # NOTE: handlers are stubs here; in Databricks they operate on Spark
                    fn(None)  # ctx placeholder
                else:
                    ptr = POINTERS.get(fm.upper())
                    if ptr:
                        print(f"[POINTER] {fm}: {ptr}")
                    else:
                        print(f"[SKIP] {fm}: no handler; likely BW standard replaced by pattern")
    ...
    def main():
        ...
        # validate
        p = sp.add_parser('validate'); p.add_argument('--config', required=False); p.add_argument('--lineage', required=False); p.set_defaults(fn=cmd_validate)
        # test
